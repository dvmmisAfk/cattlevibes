"use client";

import Image from "next/image";
import { motion, useReducedMotion } from "framer-motion";
import { FoldText } from "@/components/ui/FoldText";
import { images } from "@/data/site";

export function WhyCattleVibesHero() {
  const prefersReduced = useReducedMotion();

  return (
    <section
      className="relative flex min-h-screen min-h-[100svh] items-center overflow-hidden bg-deep-navy pt-24 pb-16 sm:pt-28 sm:pb-20 lg:pt-28 lg:pb-20 select-none"
      data-theme="dark"
      aria-labelledby="why-cattlevibes-hero-title"
    >
      {/* ─── 1. Full-Bleed Authentic Pastoral Landscape Image (Original About Hero) ─── */}
      <div className="absolute inset-0 select-none overflow-hidden" aria-hidden="true">
        <Image
          src={images.aboutHero}
          alt="Agricultural field landscape at sunrise"
          fill
          priority
          sizes="100vw"
          className="object-cover object-[center_60%] lg:object-[center_50%]"
        />
      </div>

      {/* ─── 2. Directional Tonal Scrim on Left (Protects Left Typography, Preserves Right Pasture) ─── */}
      <div
        className="pointer-events-none absolute inset-0 bg-gradient-to-r from-deep-navy/92 via-deep-navy/65 via-50% to-transparent lg:from-deep-navy/90 lg:via-deep-navy/52 lg:via-55% lg:to-transparent"
        aria-hidden="true"
      />
      <div
        className="pointer-events-none absolute inset-0 bg-gradient-to-t from-deep-navy/60 via-transparent to-deep-navy/20 lg:hidden"
        aria-hidden="true"
      />

      {/* ─── 3. Left-Aligned Editorial Content Block ─── */}
      <div className="relative z-10 mx-auto w-full max-w-[1800px] px-4 sm:px-6 lg:px-8">
        <div className="max-w-3xl lg:max-w-[850px] xl:max-w-[920px]">
          {/* Main Headline with FoldText */}
          <h1
            id="why-cattlevibes-hero-title"
            className="font-heading text-[clamp(2.25rem,5.6vw,5.5rem)] font-extrabold leading-[1.06] tracking-[-0.03em] text-white"
          >
            <FoldText
              text="Built Around Better"
              trigger="mount"
              hinge="top"
              duration={0.7}
              stagger={0.035}
              creaseShading={0.45}
            />
            <span className="block mt-1 sm:mt-2">
              <FoldText
                text="Animal Healthcare."
                trigger="mount"
                hinge="top"
                duration={0.7}
                stagger={0.035}
                creaseShading={0.45}
              />
            </span>
          </h1>

          {/* Verified Supporting Copy */}
          <motion.p
            className="mt-6 sm:mt-7 max-w-2xl text-base sm:text-lg lg:text-[1.125rem] font-medium leading-relaxed tracking-[-0.01em] text-white/90 antialiased"
            initial={prefersReduced ? {} : { opacity: 0, y: 12 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.7, delay: 0.25, ease: [0.16, 1, 0.3, 1] }}
          >
            Precision veterinary medicines and clinical nutritional solutions engineered around daily farm realities and long-term herd productivity.
          </motion.p>
        </div>
      </div>
    </section>
  );
}
