...
export function FoldText({
  ...
  const segments = useMemo(() => {
    ...
    const renderSegment = (content: React.ReactNode, key: string, split: FoldTextSplitBy = splitBy) => {
      segmentIndex += 1;
      return (
        <span
    ...
    return text.split(/(\s+)/).flatMap((part, wordIndex) => {
      ...
      return (
        <span className="fold-text-word" key={`word-${wordIndex}`}>
          {Array.from(part).map((char, charIndex) =>
            renderSegment(char, `segment-char-${wordIndex}-${charIndex}`, "char")
          )}
        </span>
      );
    });
